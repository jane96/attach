(function(){
     ...
     * @private  data-btn="user"去掉user
     */
    _deleteOperateBtnUser= function(){
        var dataVal=$('span').data('btn');
        if(dataVal=='user'){
            $(".operate-btn").eq(0).hide();
        }
    };
    /**
     * 去掉 下拉
     ...
     * @private
     */
    _deleteDown = function (){
        var dataDown = $('span').data('drop');
        if(dataDown =='none'){
            $(".select2-results").hide()
            $(".select2-choices").css("border-radius","4px");
        }
    }
...
//			}
	...
	SelectBox.prototype = {
		...
		_createTree: function(type, data) {
			...
			var that = this,
				...
				treeClick = function(evt, treeid, node) {
					...
					// 当用户可选择时，点击树项为刷新右侧列表
					if(userSelectable){   // node.id是树的id
		 ...
		 * @return {Object}    当前调用对象       这个方法改动态上传id
		 */
			...
			var that = this,
				...
				formatResult = function(data, $ct) {
					var $results = that.$element.data().select2.results,
						...
						departmentTpl = '<li class="select2-department">'  + '</li>',
				...
				query = function(query) {
					...
					that.data = getnewdata(query.term);
...
//			$operateWrap.append($openBoxBtn);
		 ...
		 * @chainable
		 * @return {Object} 当前调用对象          传回的text/name加载在页面
		 */
		...
		addValue: function(val, slient) {
			...
			if(typeof val !== "undefined"){
				...
				var aa = $.find("li[class=select2-search-choice]");
				var namearray = [];
				for(var i = 0 ; i < aa.length ; i++)
				{
					var html_code = aa[i].innerHTML;
					html_code = html_code.substring(html_code.indexOf("</i>")+("</i>".length),html_code.indexOf("</div>"));
					namearray.push(html_code);
				}
				this.select.val(this._fixEmptyArray(this.values));
				var aa1 = $.find("li[class=select2-search-choice]");
				for(var i = 0 ; i < aa.length ; i++)
				{
					var inner_html = aa1[i].innerHTML;
					//aa1[i].innerHTML = inner_html.substring(0,inner_html.indexOf("</i>")+1) + namearray[i] + inner_html.substring("</div>");
					aa1[i].innerHTML = inner_html.substring(0,inner_html.indexOf("</i>")+("</i>".length))+ namearray[i] +inner_html.substring(inner_html.indexOf("</div>"))
					$(aa1[i]).attr("id","ddd_"+i).find("a").bind("click",function(){
						var index_number = $(this).parent().attr("id").replace("ddd_","");
						var values = $("#taskRole").val().split(",");
						var newvalue = "";
						var delvalue="";
						all_this.values = [];
						if(values.length > 2)
		...
		removeValue: function(val, slient) {
			var newval = "";
			var oldvals = $("#taskRole").val().split(",");
			for(var i = 0 ; i < oldvals.length ; i++)
			{
				if(oldvals[i] != val && oldvals[i] != null && oldvals[i] != '')
				{
					newval += oldvals[i] + ",";
				}
			}
	...
	function getnewdata(searchQuery)
	{
		var allData = [];
		$.ajax({
            url:localurl+"/a/sys/dict/child1/role_type",
            type:"GET",
            ...
            success:function(data){
                for(var i = 0 ;i<data.length;i++){
                    ...
                    data[i].iconSkin="position";
                    data[i].type = "position";
                    data[i].enable = 1 ;
                    data[i].checked = true;
                    if(data[i].enable == 1){
                        allData.push(data[i]);
