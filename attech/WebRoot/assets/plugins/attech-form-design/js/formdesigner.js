	...
	formEdition: {
		...
		preview: function(id) {
			if (this._checkEdition()) {
				window.open(Attech.app.url('workflow/formtype/preview', {verid: id}),
		...
		restore: function(id) {
			if (this._checkEdition()) {
				Ui.confirm(U.lang("WF.EDITION_RESTORE_CONFIRM"), function() {
					$.get(Attech.app.url('workflow/formversion/restore'), {verid: id}, function(res) {
						if (res.isSuccess) {
		...
		remove: function(id) {
			if (this._checkEdition()) {
				Ui.confirm(U.lang("WF.EDITION_DELETE_CONFIRM"), function() {
					$.get(Attech.app.url('workflow/formversion/del'), {verid: id}, function(res) {
						if (res.isSuccess) {
