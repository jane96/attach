(function(){
	...
	var SelectBox = function($element, options) {
		if(!Attech || !Attech.data){
			$.error("(SelectBox): 未定义数据Attech.data");
		}
	...
	SelectBox.defaults = {
		...
		navSettings: [{
				...
				data: Attech.data && Attech.data.get("department") || [],
				type: "department"
			}, {
				// id: "select_box_position",
				icon: "select-box-position",
				text: U.lang("US.PER_POSITION"),
				data: Attech.data && (Attech.data.get("position").concat(Attech.data.get("positioncategory"))) || [],
				type: "position"
			},
			{
				...
				data: Attech.data && Attech.data.get("contact") || [],
				type: "contact"
			}
		],
		values: [],
	...
	var UserSelect = function($element, options) {
		// @Debug:
		if (!Attech || !Attech.data) {
			throw new Error("(SelectBox): 未找到全局数据Attech.data")
		}
