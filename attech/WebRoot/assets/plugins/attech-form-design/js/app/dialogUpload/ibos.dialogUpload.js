...
(function(){
	var DProgress = function(file, targetId, opts){
		...
		if(!this.element.length) {
			...
			this.element.html($.template(_template, {
				icon: "",
				name: Attech.string.ellipsis(file.name, 20, 10),
	...
	var _upload = Attech.dialogUpload =  function(opts) {
		var swfPath, uploadInstance;uploadInstance
		if(typeof SWFUpload !== "undefined") {
			Attech.dialogUploadInstance = Attech.upload.attach($.extend(true, {
		        button_image_url: "",
			...
			swfPath = Attech.app.getStaticUrl("/js/lib/SWFUpload");
