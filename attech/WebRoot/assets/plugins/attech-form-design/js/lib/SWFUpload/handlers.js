...
(function(){
	...
	SWFUpload.defaults = {
		...
		// Button Settings
		button_image_url:                           Attech.app.getStaticUrl("/image/upload_btn.png"),
		...
		// Flash Settings
		flash_url:                                  Attech.app.getStaticUrl("/js/lib/SWFUpload/swfupload_fp10/swfupload.swf"),
		flash9_url:                                 Attech.app.getStaticUrl("/js/lib/SWFUpload/swfupload_fp9/swfupload.swf")
	}
	SWFUpload.getErrorInfo = getErrorInfo;

	Attech.fileUpload = function(options){
	...
	Attech.imgUpload = function(options){
		var $movie,
			$wrap,
			$cover,
			$progress,
		...
		return Attech.fileUpload($.extend({}, imgUploadSettings, options));
	}
	...
	var _attachLoaded = function(){
		...
		if(!$container.length) {
			$.error("Attech.upload.attach: 未找到节点#" + cs.containerId);
		}
	...
	Attech.upload.attach = function(options){
		...
		var _settings = {
			// Backend Settings
			upload_url:                                 Attech.app.url('main/attach/upload', { "uid": Attech.app.g("uid"), "hash": uploadConf.hash}),
			...
			button_image_url:                        	Attech.app.getStaticUrl("/image/upload_btn_attach.png"),
			button_placeholder_id:                  	"upload_btn",
			button_width:                           	40,
			button_height:                          	40,
		...
		return Attech.fileUpload($.extend(true, {}, _settings, options))
	}
	...
	Attech.upload.image = function(options){
		var uploadConf = $.extend(true, {
			max: 0,
			imageexts: {
