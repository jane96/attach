...
var ctrlsdata = {
        ...
        'icimgupload': { title: "上传控件" },
        /*'icprogressbar': { title: "进度条控件" },*/
        'icsign': { title: "签章控件" },
        'icqrcode': { title: "二维码控件" },
    };
var bussiId = art.dialog.data("bussiId");
var templateId = art.dialog.data("templateId");
if("" == art.dialog.data("tem")){
	$.ajax({
		type : "get",
		url : "/a/form/propenum/getPropEnum?bussiType=" + bussiId,
		async : false,
		dataType : "json",
		success : function(data) {
			if (data.status == "success") {
				...
				for(var i = 0; i<datas.length; i++){
					$("#propcommon").append(datas[i].formdata);
					var name = datas[i].ctrlsdata;
					var oldname = datas[i].propdata;
					alldata[name] = alldata[oldname];
					ctrlsdata[name] = ctrlsdata[oldname];
				}
			}
		}
	});
	...
	$.ajax({
		type : "get",
		url : "/a/form/propenum/getPropEnum?bussiType=" + bussiId + "&tempId=" + templateId,
		async : false,
		dataType : "json",
		success : function(data) {
			if (data.status == "success") {
				var datas = data.datas;
				for(var i = 0; i<datas.length; i++){
					if(undefined == datas[i].exist || "" == datas[i].exist)$("#propcommon").append(datas[i].formdata);
					var name = datas[i].ctrlsdata;
					var oldname = datas[i].propdata;
					alldata[name] = alldata[oldname];
					ctrlsdata[name] = ctrlsdata[oldname];
...
(function(){
    ...
    var timerCount = setInterval(function(){
        if(UE.I18N['zh-cn']) {
            UE.utils.extend(UE.I18N['zh-cn'], alldata);
            ...
            UE.commands[id] = {
                ...
                execCommand: function(command, elem){
                	if(command.indexOf(".") > 0){
                		...
                		UE.plugins['formcontrols'].propEnumId = commands[1];
                		command = commands[0];
                	}
                    var id = command,
                    ...
                    console.info("qqqqqqqqqqqqqqqqqqqq" + id);
                    // editing 属性指向编辑中的节点
                    ...
                    if(!dialog) {
                        dialog = new UE.ui.Dialog({
                            ...
                            iframeUrl: me.ui.mapUrl('~/dialogs/' + id + '/' + id + '.shtml?=' + Math.random()),
                            editor: me,
                            ...
                            buttons: [{
                                    ...
                                    onclick: function(){
                                        if(window.confirm("确定删除该控件吗？")) {
                                        	var tpl = '<ic data-id="<%=id%>"';
                                        	fc = new Fc(editor, tpl);
                                        	var oldData = fc.getControlData(UE.plugins['formcontrols'].editing);
                                        	$.ajax({
                                        		type : 'post',
                                        		datatype : 'json',
                                        		url : '/a/form/template/elementdel/' + templateId + "/" + oldData.id,
                                        		success : function(data) {
                                        			if(data.status == "success"){
                                        				if(undefined != data.remarks){
                                        					$("#propcommon").append(data.remarks);
                                        				}
                                        				UE.plugins['formcontrols'].editing &&
                                                        UE.dom.domUtils.remove(UE.plugins['formcontrols'].editing, false);
                                                        dialog.close(false);
                                        			}else{
                                        				alert("未找到该表单元素或者系统错误，请重试！");
                                        			}
                                        		}
