$(function(){
	var Call = {
		...
		selectMember: function(selectBtn, $selectBox, $initInput){
			...
			$selectBox.selectBox({
				data: Attech.data && Attech.data.get(),
				type: 'user',
		...
		formatUserInfo: function(param){
			...
			// 外部人员, 为使用符号分隔的多个电话号码
				...
				var avatar = Attech.app.g("emptyAvatar");
	...
	Call.connect = function(param, mode) {
		var promise = $.get(Attech.app.url('main/call/chkConf'), function(res){
			if( res.isSuccess ){
				var info = [];
				...
				// 根据类型拼接出完整路径
				var url = Attech.app.url(routes[mode], { data: info });
				//新窗打开通话中界面
