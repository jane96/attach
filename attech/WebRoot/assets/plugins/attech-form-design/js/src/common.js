...
var U = Attech.Util = (function(){
	...
	U.setCookie = function(name, value, seconds, path, domain, secure) {
		...
		path = path || Attech.app.g('cookiePath') || "";
		domain = domain || Attech.app.g('cookieDomain') || "";
		expires.setTime(expires.getTime() + seconds * 1000);
		document.cookie = escape((Attech.app.g('cookiePre') || "") + name) + '=' + escape(value) + 
	...
	U.getCookie = function(name, nounescape) {
		name = (Attech.app.g('cookiePre') || "") + name;
...
// Debug
Attech.print = function(){ console && console.log(arguments); };
Attech.time = function(name, time, func){
...
Attech.app = (function(){
	 ...
	 * @example 
	 *  	Attech.app.url('main/default/index');
	 *  	// ==> localhost/?r=main/default/index
	 *  	Attech.app.url('main/default/index', { op: "add" });
	 ...
	 * @example
	 * 	Attech.app.getAssetUrl("main", "/js/index.js");
	 * @return
	 */
...
Attech.settings = {
	...
	dataTable: {
		...
		// --- Language
		language: $.extend({}, Attech.l("DATATABLE"), {
			processing: '<img src="' + Attech.app.getStaticUrl("/image/loading.gif") + '">' //"加载中..."
		}),
...
// @deprecated 
Attech.Event = function($ctx, type, flag){
	var that = this;
	type = type || "click";
	flag = flag || type;
	...
	$ctx.on(type, "[data-" + flag + "]", function(){
		var evtName = $.attr(this, "data-" + flag),
			params = Attech.app.getEvtParams(this);
		that.fire(evtName, params, $(this));
	})
	this._evts = {};
}

Attech.Event.prototype = {
	constructor: Attech.Event,
...
Attech.events = new Attech.Event();
...
(function(window, $){
	 ...
	 * @static
	 */
	window.Dom = Attech.Dom = (function(){
		var Dom = {
			/**
	...
	(function(){
		...
		Attech.core.inherits(Ui.PopMenu, Ui.Menu);
		Ui.PopMenu.defaults = {
			position: {
				at: "left bottom",
				my: "left top"
		...
		Ui.showCreditPrompt = function() {
			...
			if(!Attech.app.g('uid') || remind.length < 2 || remind[6] !== Attech.app.g('uid')) {
				_reset();
				return;
			}

			names = Attech.app.g('creditRemind').split(',');
	...
	$.extend(Attech, {
		...
		atwho: function($ctx, options){
			...
			opts = $.extend({
				at: "@",
				data: Attech.data && Attech.data.get("user") || [],
				...
				callbacks: {
					filter: function(query, data, search_key){
						...
						for (_i = 0, _len = data.length; _i < _len; _i++) {
							...
							else if(Attech.matchSpell(query, item[search_key])){
								_results.push(item);
							}
						}
						return _results
		...
		intro: function(steps, complete){
			...
			if(typeof introJs === "undefined") {
				$.error("Attech.intro: 未引入intro.js");
			}
			ito = introJs();
			ito.setOptions({
				steps: steps,
	 ...
	 * @return
	 */
	Attech.checkFormChange = function(context, tip, selectors){
...
$.fn.waiting = function(content, mode, lock){
	...
	content = content || "";
	mode = mode || "normal";
	var tpl = '<div><img src="<%= Attech.app.getStaticUrl(path) %>" width="<%= size %>" height="<%= size %>" />' +
...
// @deprecated 使用 Attech.cmList 或 dataTable 替换
Attech.List = (function(){
		...
		_assignId: function(data){
			...
			if(!data[idField]){
				...
				if(this.hasId(data[idField])) {
					$.error("Attech.List._assignId: " + id + "重复");
				}
			}
			return data;
		},
...
// @deprecated 使用 Attech.cmList 或 dataTable 替换
Attech.OrderList = function($elem, tpl, options){
	options = {} || options;
...
Attech.core.inherits(Attech.OrderList, Attech.List);

$.extend(Attech.OrderList.prototype, {
	addItem: function(data, prepend) {
		data = $.extend({
			index: this.index
		}, data);
...
// 通用列表类，提供列表的增删除改功能
Attech.CmList = function(container, opts){
	this.$container = $(container);
	this.opts = $.extend({}, this.constructor.defaults, opts);
}
Attech.CmList.prototype = {
	constructor: Attech.CmList,
