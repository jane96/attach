...
var searchTemplate = {
	_wid: 0,
	addUrl: Attech.app.url('workflow/querytpl/add'),
	editUrl: Attech.app.url('workflow/querytpl/edit'),
	delUrl: Attech.app.url('workflow/querytpl/del'),
	listUrl: Attech.app.url('workflow/querytpl/index'),
	showList: function(param, ok, cancel) {
		this._wid = guide.getId();
		guide.dialog(this.listUrl, param, {
			title: Attech.l('WF.FLOW_QUERY_TPL'),
			ok: false,
			cancel: true
		});
	},
	showSetting: function(param, ok, cancel) {
		...
		if (param) {
			url = this.editUrl;
			title = Attech.l('WF.EDIT_QUERY_TPL');
		} else {
			url = this.addUrl;
			title = Attech.l('WF.NEW_QUERY_TPL');
		}
		guide.dialog(url, param, {
			...
			ok: function() {
				...
				if ($conditionSelect.length > 0) {
					...
					if (!checkExp) {
						Ui.tip(Attech.l('WF.CONDITION_FORMAT_ERROR'), 'danger');
						if (tab) {
				...
				$form.waiting(Attech.l('IN_SUBMIT'), "mini", true);
				$.post(url, $form.serializeArray(), function(data) {
					$form.stopWaiting();
					if (data.isSuccess) {
						Ui.tip(Attech.l('SAVE_SUCEESS'), 'success');
						that.showList();
					} else {
						Ui.tip(Attech.l('SAVE_FAILED'), 'danger');
					}
				}, 'json');
				return false;
			},
...
var Manager = {
	_wid: 0,
	AddUrl: Attech.app.url('workflow/manager/add'),
	EditUrl: Attech.app.url('workflow/manager/edit'),
	listUrl: Attech.app.url('workflow/manager/index'),
	deleteUrl: Attech.app.url('workflow/manager/del'),
	showList: function(param) {
		this._wid = guide.getId();
		guide.dialog(this.listUrl, param, {
			title: Attech.l('WF.MANAGER_LIST'),
			ok: false,
			cancel: true
		});
	},
	showSetup: function(param) {
		...
		if (param) {
			url = this.EditUrl;
			title = Attech.l('WF.EDIT_MANAGE_PRIV');
		} else {
			url = this.AddUrl;
			title = Attech.l('WF.NEW_MANAGE_PRIV');
		}
		guide.dialog(url, param, {
			title: title,
			ok: function() {
				if ($.trim($('#auth_for').val()) === '') {
					Ui.tip(Attech.l('WF.INVALID_AUTH_OBJECT'), 'danger');
					return false;
				}
				if ($('#manage_scope').val() === 'custom' && $('#custom_department_select').val() === '') {
					Ui.tip(Attech.l('WF.INVALID_CUSTOM_DEPT'), 'danger');
					return false;
				}
				$('#auth_rule_setting').waiting(Attech.l('IN_SUBMIT'), "mini", true);
...
var timer = {
	_wid: 0,
	listUrl: Attech.app.url('workflow/timer/index'),
	saveUrl: Attech.app.url('workflow/timer/save'),
	showList: function(param, ok, cancel) {
		var that = this;
		guide.dialog(this.listUrl, param, {
			title: Attech.l('WF.SET_THE_TIMING_TASK'),
			ok: function() {
				$('#crontab_form').waiting(Attech.l('IN_SUBMIT'), "mini", true);
...
var free = {
	_wid: 0,
	url: Attech.app.url('workflow/type/freenew'),
	show: function(param) {
		var that = this;
		this._wid = guide.getId();
		guide.dialog(this.url, param, {
			id: 'free_new_user',
			title: Attech.l('WF.SPECIFY_FREE_FLOW_NEW_USER'),
			ok: function() {
				var val = $('#new_user').val();
				if ($.trim(val) !== '') {
					$.post(that.url, {flowid: that._wid, formhash: G.formHash, newuser: val}, function(data) {
						if (data.isSuccess) {
							Ui.tip(Attech.l('OPERATION_SUCCESS'), 'success');
							guide.hideDialog();
						}
					}, 'json');
				}
...
(function() {
	var handler = {
		editFlow: function(flowid) {
			window.location.href = Attech.app.url('workflow/type/edit', {flowid: flowid});
		},
		importFlow: function(flowid) {
			Ui.openFrame(Attech.app.url('workflow/type/import', {flowid: flowid}), {
				id: 'import_frame',
				title: Attech.l('WF.SPECIFY_THE_IMPORT_FILE'),
				padding: 20,
				ok: function() {
					var ifm = this.iframe;
				...
				okVal: Attech.l("IMPORT"),
				cancel: true
			});
		},
		exportFlow: function(flowid) {
			window.location.href = Attech.app.url('workflow/type/export', {flowid: flowid});
		},
		designForm: function() {
			window.open(Attech.app.url('workflow/formtype/design', {formid: guide.getFormId()}), "designForm");
		},
		previewForm: function() {
			window.open(Attech.app.url('workflow/formtype/preview', {formid: guide.getFormId()}), "previewForm");
		},
		designFlow: function(flowid) {
			window.open(Attech.app.url('workflow/process/index', {flowid: flowid}), "designFlow");
		},
		verifyFlow: function(flowid) {
			var opt = {
				title: Attech.l('WF.EXAM_FLOW'),
				id: 'd_verify',
				padding: 0,
				cancel: true
			};
			var url = Attech.app.url('workflow/type/verify', {flowid: flowid});
